	...
	public class AssetLibraryBundle extends EventDispatcher
	{
		...
		public function addAsset(asset:IAsset):void
		{
			...
			if (!_assetDictionary.hasOwnProperty(ns))
            {
                ...
                _assetDictionary[ns] = new Object();
		...
		public function removeAllAssets(dispose:Boolean = true):void
		{
			if (dispose)
            {
                ...
                for ( var c : Number = 0 ; c < _assets.length ; c ++ )
                {
                    asset = _assets[ c ];
                    asset.dispose();
		...
		public function stopAllLoadingSessions():void
		{
			...
			var length:Number = _loadingSessions.length;
		...
		{
			if (hasEventListener(LoaderEvent.LOAD_ERROR , onDependencyRetrievingError , this ) )
            {
		...
		{
			if (hasEventListener(ParserEvent.PARSE_ERROR, onDependencyRetrievingParseError, this ))
            {
		...
		private function onResourceRetrieved(event:LoaderEvent):void
		{
            ...
            var index:Number = _loadingSessions.indexOf(loader);
        ...
        private function loadingSessionGC():void
        {
            ...
            while( _loadingSessionsGarbage.length > 0 )
            {

                loader = _loadingSessionsGarbage.pop();
                killLoadingSession( loader );

            }

            Window.clearTimeout(_gcTimeoutIID);
            _gcTimeoutIID = null;
