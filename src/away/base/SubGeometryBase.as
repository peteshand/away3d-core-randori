...
package away.base
{
	...
	public class SubGeometryBase
	{
		...
		public function getIndexBuffer(stage3DProxy:Stage3DProxy):IndexBuffer3D
		{
            ...
            {
                _indexBuffer[contextIndex].uploadFromArray(_indices, 0, _numIndices);
		...
		public function pUpdateFaceTangents():void
		{
			...
			var posStride:Number = vertexStride;
			var posOffset:Number = vertexOffset;
			var texStride:Number = UVStride;
			var texOffset:Number = UVOffset;

            if ( _faceTangents == null)
            {
                _faceTangents = new Vector.<Number>( _indices.length ); //||= new Vector.<Number>(_indices.length, true);
            }
		...
		private function updateFaceNormals():void
		{
            ...
            {
                _faceNormals = new Vector.<Number>( len );//_faceNormals ||= new Vector.<Number>(len, true);
            }
			...
			if ( _useFaceWeights )
            {
                ...
                {
                    _faceWeights = new Vector.<Number>( len / 3 );//_faceWeights ||= new Vector.<Number>(len/3, true);
                }
            }
		...
		public function pUpdateVertexNormals(target:Vector.<Number>):Vector.<Number>
		{
            ...
            var f2:Number = 1;
            var f3:Number = 2;
			var lenV:Number = _vertexData.length;
			var normalStride:Number = vertexNormalStride;
			var normalOffset:Number = vertexNormalOffset;
		...
		public function pUpdateVertexTangents(target:Vector.<Number>):Vector.<Number>
		{
            ...
            {
                pUpdateFaceTangents()//updateFaceTangents();
            }

			var i:Number;
			var lenV:Number = _vertexData.length;
			var tangentStride:Number = vertexTangentStride;
			var tangentOffset:Number = vertexTangentOffset;
		...
		{
			pDisposeIndexBuffers(_indexBuffer);
            _indices = null;
		...
		public function updateIndexData(indices:Vector.<Number>/*uint*/):void
		{
			...
			var numTriangles:Number = _numIndices/3;
            ...
            pInvalidateBuffers(_indicesInvalid);
            _faceNormalsDirty = true;
		...
		public function pInvalidateBounds():void
		{
            ...
            {
                var me : *  = this;
                _parentGeometry.iInvalidateBounds( (me as ISubGeometry));
            }
		...
		{
			var offset:Number = UVOffset;
			var stride:Number = UVStride;
			var uvs:Vector.<Number> = UVData;
		...
		public function scale(scale:Number):void
		{
			...
			var offset:Number = vertexOffset;
			var stride:Number = vertexStride;
		...
		public function pUpdateDummyUVs(target:Vector.<Number>):Vector.<Number>
		{
			...
			var len:Number = _vertexData.length/vertexStride*stride;
