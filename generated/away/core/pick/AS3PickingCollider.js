...
away.core.pick.AS3PickingCollider = function(findClosestCollision) {
	this._findClosestCollision = false;
	away.core.pick.PickingColliderBase.call(this);
	this._findClosestCollision = findClosestCollision;
};

away.core.pick.AS3PickingCollider.prototype.testSubMeshCollision = function(subMesh, pickingCollisionVO, shortestCollisionDistance) {
	var t;
	var i0, i1, i2;
	var rx, ry, rz;
	var nx, ny, nz;
	...
	for (var index = 0; index < numIndices; index += 3) {
		...
		if ((!bothSides && nDotV < 0.0) || (bothSides && nDotV != 0.0)) {
			...
			if (!(u < 0) && t > 0 && t < shortestCollisionDistance) {
				...
				pickingCollisionVO.localPosition = new away.core.geom.Vector3D(cx, cy, cz, 0);
				pickingCollisionVO.localNormal = new away.core.geom.Vector3D(nx, ny, nz, 0);
...
$inherit(away.core.pick.AS3PickingCollider, away.core.pick.PickingColliderBase);

away.core.pick.AS3PickingCollider.className = "away.core.pick.AS3PickingCollider";

away.core.pick.AS3PickingCollider.getRuntimeDependencies = function(t) {
	var p;
	p = [];
...
away.core.pick.AS3PickingCollider.getStaticDependencies = function(t) {
	var p;
	return [];
};

away.core.pick.AS3PickingCollider.injectionPoints = function(t) {
	var p;
	switch (t) {
		case 0:
			p = [];
			...
			break;
		case 1:
			p = away.core.pick.PickingColliderBase.injectionPoints(t);
			break;
		case 2:
			p = away.core.pick.PickingColliderBase.injectionPoints(t);
			break;
