...
away.core.pick.ShaderPicker = function() {
	this._hitRenderable = null;
	this._context = null;
	this._triangleProgram3D = null;
	this.MOUSE_SCISSOR_RECT = new away.core.geom.Rectangle(0, 0, 1, 1);
	this._potentialFound = false;
	this._hitColor = 0;
	this._id = null;
	this._interactiveId = 0;
	this._rayPos = new away.core.geom.Vector3D(0, 0, 0, 0);
	this._subGeometryIndex = 0;
	this._faceIndex = 0;
	this._rayDir = new away.core.geom.Vector3D(0, 0, 0, 0);
	this._interactives = [];
	this._stage3DProxy = null;
	this._localHitNormal = new away.core.geom.Vector3D(0, 0, 0, 0);
	this._onlyMouseEnabled = true;
	this._hitUV = new away.core.geom.Point(0, 0);
	...
	this._projY = 0;
	this._projX = 0;
	this._localHitPosition = new away.core.geom.Vector3D(0, 0, 0, 0);
...
away.core.pick.ShaderPicker.MOUSE_SCISSOR_RECT = new away.core.geom.Rectangle(0, 0, 1, 1);

away.core.pick.ShaderPicker.prototype.get_onlyMouseEnabled = function() {
	return this._onlyMouseEnabled;
};

away.core.pick.ShaderPicker.prototype.set_onlyMouseEnabled = function(value) {
	this._onlyMouseEnabled = value;
};

away.core.pick.ShaderPicker.prototype.getViewCollision = function(x, y, view) {
	...
	this._viewportData[3] = this._projY;
	this._potentialFound = false;
	...
	if (!this._bitmapData)
		this._bitmapData = new away.core.display.BitmapData(1, 1, false, 0);
...
away.core.pick.ShaderPicker.prototype.getSceneCollision = function(position, direction, scene) {
	return null;
};

away.core.pick.ShaderPicker.prototype.pDraw = function(entityCollector, target) {
	var camera = entityCollector.get_camera();
	this._context.clear(0, 0, 0, 1, 1, 0, 17664);
	this._stage3DProxy.set_scissorRect(away.core.pick.ShaderPicker.MOUSE_SCISSOR_RECT);
	this._interactiveId = 0;
	this._interactives.length = this._interactiveId;
	if (!this._objectProgram3D) {
		this.initObjectProgram3D();
	}
	this._context.setBlendFactors(away.core.display3D.Context3DBlendFactor.ONE, away.core.display3D.Context3DBlendFactor.ZERO);
	this._context.setDepthTest(true, away.core.display3D.Context3DCompareMode.LESS);
	this._context.setProgram(this._objectProgram3D);
	this._context.setProgramConstantsFromArray(away.core.display3D.Context3DProgramType.VERTEX, 4, this._viewportData, 1);
...
away.core.pick.ShaderPicker.prototype.drawRenderables = function(item, camera) {
	...
	while (item) {
		...
		this._context.setCulling(renderable.get_material().get_bothSides() ? away.core.display3D.Context3DTriangleFace.NONE : away.core.display3D.Context3DTriangleFace.BACK);
		...
		this._context.setProgramConstantsFromMatrix(away.core.display3D.Context3DProgramType.VERTEX, 0, matrix, true);
		this._context.setProgramConstantsFromArray(away.core.display3D.Context3DProgramType.FRAGMENT, 0, this._id, 1);
...
away.core.pick.ShaderPicker.prototype.updateRay = function(camera) {
	this._rayPos = camera.get_scenePosition();
	this._rayDir = camera.getRay(this._projX, this._projY, 1);
	this._rayDir.normalize();
};

away.core.pick.ShaderPicker.prototype.initObjectProgram3D = function() {
	var vertexCode;
	var fragmentCode;
...
away.core.pick.ShaderPicker.prototype.initTriangleProgram3D = function() {
	var vertexCode;
	var fragmentCode;
...
away.core.pick.ShaderPicker.prototype.getHitDetails = function(camera) {
	this.getApproximatePosition(camera);
	this.getPreciseDetails(camera);
};

away.core.pick.ShaderPicker.prototype.getApproximatePosition = function(camera) {
	var entity = this._hitRenderable.get_sourceEntity();
	var col;
	var scX, scY, scZ;
	var offsX, offsY, offsZ;
	var localViewProjection = away.core.math.Matrix3DUtils.CALCULATION_MATRIX;
	...
	this._context.clear(0, 0, 0, 0, 1, 0, away.core.display3D.Context3DClearMask.DEPTH);
	this._context.setScissorRectangle(away.core.pick.ShaderPicker.MOUSE_SCISSOR_RECT);
	this._context.setProgramConstantsFromMatrix(away.core.display3D.Context3DProgramType.VERTEX, 0, localViewProjection, true);
	this._context.setProgramConstantsFromArray(away.core.display3D.Context3DProgramType.VERTEX, 5, this._boundOffsetScale, 2);
...
away.core.pick.ShaderPicker.prototype.getPreciseDetails = function(camera) {
...
away.core.pick.ShaderPicker.prototype.getPrecisePosition = function(invSceneTransform, nx, ny, nz, px, py, pz) {
	var rx, ry, rz;
	var ox, oy, oz;
	var t;
...
away.core.pick.ShaderPicker.prototype.dispose = function() {
...
away.core.pick.ShaderPicker.className = "away.core.pick.ShaderPicker";

away.core.pick.ShaderPicker.getRuntimeDependencies = function(t) {
	var p;
	p = [];
...
away.core.pick.ShaderPicker.getStaticDependencies = function(t) {
	var p;
	p = [];
...
away.core.pick.ShaderPicker.injectionPoints = function(t) {
	return [];
};
