...
away.core.display3D.Context3D = function(canvas) {
	this._currentWrap = 0;
	...
	for (var i = 0; i < away.core.display3D.Context3D.MAX_SAMPLERS; ++i) {
		this._samplerStates[i] = new away.core.display3D.SamplerState();
...
away.core.display3D.Context3D.MAX_SAMPLERS = 8;

away.core.display3D.Context3D.prototype.gl = function() {
	return this._gl;
};

away.core.display3D.Context3D.prototype.clear = function(red, green, blue, alpha, depth, stencil, mask) {
	red = red || 0;
	green = green || 0;
	blue = blue || 0;
	alpha = alpha || 1;
...
away.core.display3D.Context3D.prototype.configureBackBuffer = function(width, height, antiAlias, enableDepthAndStencil) {
...
away.core.display3D.Context3D.prototype.createCubeTexture = function(size, format, optimizeForRenderToTexture, streamingLevels) {
	streamingLevels = streamingLevels || 0;
	var texture = new away.core.display3D.CubeTexture(this._gl, size);
	this._textureList.push(texture);
	return texture;
};

away.core.display3D.Context3D.prototype.createIndexBuffer = function(numIndices) {
	var indexBuffer = new away.core.display3D.IndexBuffer3D(this._gl, numIndices);
	this._indexBufferList.push(indexBuffer);
	return indexBuffer;
};

away.core.display3D.Context3D.prototype.createProgram = function() {
	var program = new away.core.display3D.Program3D(this._gl);
	this._programList.push(program);
	return program;
};

away.core.display3D.Context3D.prototype.createTexture = function(width, height, format, optimizeForRenderToTexture, streamingLevels) {
	streamingLevels = streamingLevels || 0;
	var texture = new away.core.display3D.Texture(this._gl, width, height);
	this._textureList.push(texture);
	return texture;
};

away.core.display3D.Context3D.prototype.createVertexBuffer = function(numVertices, data32PerVertex) {
	var vertexBuffer = new away.core.display3D.VertexBuffer3D(this._gl, numVertices, data32PerVertex);
	this._vertexBufferList.push(vertexBuffer);
	return vertexBuffer;
};

away.core.display3D.Context3D.prototype.dispose = function() {
	var i;
...
away.core.display3D.Context3D.prototype.drawToBitmapData = function(destination) {
	throw new away.errors.PartialImplementationError("", 0);
};

away.core.display3D.Context3D.prototype.drawTriangles = function(indexBuffer, firstIndex, numTriangles) {
...
away.core.display3D.Context3D.prototype.present = function() {
	this._drawing = false;
};

away.core.display3D.Context3D.prototype.setBlendFactors = function(sourceFactor, destinationFactor) {
	this._blendEnabled = true;
	switch (sourceFactor) {
		case away.core.display3D.Context3DBlendFactor.ONE:
			this._blendSourceFactor = 1;
			break;
		case away.core.display3D.Context3DBlendFactor.DESTINATION_ALPHA:
			this._blendSourceFactor = 772;
			break;
		case away.core.display3D.Context3DBlendFactor.DESTINATION_COLOR:
			this._blendSourceFactor = 774;
			break;
		case away.core.display3D.Context3DBlendFactor.ONE:
			this._blendSourceFactor = 1;
			break;
		case away.core.display3D.Context3DBlendFactor.ONE_MINUS_DESTINATION_ALPHA:
			this._blendSourceFactor = 773;
			break;
		case away.core.display3D.Context3DBlendFactor.ONE_MINUS_DESTINATION_COLOR:
			this._blendSourceFactor = 775;
			break;
		case away.core.display3D.Context3DBlendFactor.ONE_MINUS_SOURCE_ALPHA:
			this._blendSourceFactor = 771;
			break;
		case away.core.display3D.Context3DBlendFactor.ONE_MINUS_SOURCE_COLOR:
			this._blendSourceFactor = 769;
			break;
		case away.core.display3D.Context3DBlendFactor.SOURCE_ALPHA:
			this._blendSourceFactor = 770;
			break;
		case away.core.display3D.Context3DBlendFactor.SOURCE_COLOR:
			this._blendSourceFactor = 768;
			break;
		case away.core.display3D.Context3DBlendFactor.ZERO:
			this._blendSourceFactor = 0;
			break;
		default:
...
away.core.display3D.Context3D.prototype.setColorMask = function(red, green, blue, alpha) {
	this._gl.colorMask(red, green, blue, alpha);
};

away.core.display3D.Context3D.prototype.setCulling = function(triangleFaceToCull) {
	if (triangleFaceToCull == away.core.display3D.Context3DTriangleFace.NONE) {
...
away.core.display3D.Context3D.prototype.setDepthTest = function(depthMask, passCompareMode) {
	switch (passCompareMode) {
		case away.core.display3D.Context3DCompareMode.ALWAYS:
			this._gl.depthFunc(519);
			break;
		case away.core.display3D.Context3DCompareMode.EQUAL:
			this._gl.depthFunc(514);
			break;
		case away.core.display3D.Context3DCompareMode.GREATER:
			this._gl.depthFunc(516);
			break;
		case away.core.display3D.Context3DCompareMode.GREATER_EQUAL:
			this._gl.depthFunc(518);
			break;
		case away.core.display3D.Context3DCompareMode.LESS:
			this._gl.depthFunc(513);
			break;
		case away.core.display3D.Context3DCompareMode.LESS_EQUAL:
			this._gl.depthFunc(515);
			break;
		case away.core.display3D.Context3DCompareMode.NEVER:
			this._gl.depthFunc(512);
			break;
		case away.core.display3D.Context3DCompareMode.NOT_EQUAL:
...
away.core.display3D.Context3D.prototype.setProgram = function(program3D) {
	this._currentProgram = program3D;
	program3D.focusProgram();
};

away.core.display3D.Context3D.prototype.getUniformLocationNameFromAgalRegisterIndex = function(programType, firstRegister) {
	switch (programType) {
		case away.core.display3D.Context3DProgramType.VERTEX:
			return "vc";
			break;
		case away.core.display3D.Context3DProgramType.FRAGMENT:
			return "fc";
			break;
		default:
...
away.core.display3D.Context3D.prototype.setProgramConstantsFromMatrix = function(programType, firstRegister, matrix, transposedMatrix) {
...
away.core.display3D.Context3D.modulo = 0;

away.core.display3D.Context3D.prototype.setProgramConstantsFromArray = function(programType, firstRegister, data, numRegisters) {
...
away.core.display3D.Context3D.prototype.setGLSLProgramConstantsFromMatrix = function(locationName, matrix, transposedMatrix) {
...
away.core.display3D.Context3D.prototype.setGLSLProgramConstantsFromArray = function(locationName, data, startIndex) {
	startIndex = startIndex || 0;
	var location = this._gl.getUniformLocation(this._currentProgram.get_glProgram(), locationName);
	this._gl.uniform4f(location, data[startIndex], data[startIndex + 1], data[startIndex + 2], data[startIndex + 3]);
};

away.core.display3D.Context3D.prototype.setScissorRectangle = function(rectangle) {
	if (!rectangle) {
...
away.core.display3D.Context3D.prototype.setTextureAt = function(sampler, texture) {
	var locationName = "fs" + sampler;
	this.setGLSLTextureAt(locationName, texture, sampler);
};

away.core.display3D.Context3D.prototype.setGLSLTextureAt = function(locationName, texture, textureIndex) {
	if (!texture) {
...
away.core.display3D.Context3D.prototype.setSamplerStateAt = function(sampler, wrap, filter, mipfilter) {
	var glWrap = 0;
	var glFilter = 0;
	var glMipFilter = 0;
	switch (wrap) {
		case away.core.display3D.Context3DWrapMode.REPEAT:
			glWrap = 10497;
			break;
		case away.core.display3D.Context3DWrapMode.CLAMP:
			glWrap = 33071;
			break;
		default:
	...
	switch (filter) {
		case away.core.display3D.Context3DTextureFilter.LINEAR:
			glFilter = 9729;
			break;
		case away.core.display3D.Context3DTextureFilter.NEAREST:
			glFilter = 9728;
			break;
		default:
	...
	switch (mipfilter) {
		case away.core.display3D.Context3DMipFilter.MIPNEAREST:
			glMipFilter = 9984;
			break;
		case away.core.display3D.Context3DMipFilter.MIPLINEAR:
			glMipFilter = 9987;
			break;
		case away.core.display3D.Context3DMipFilter.MIPNONE:
			glMipFilter = 0;
			break;
		default:
			throw "MipFilter is not supported " + mipfilter;
	}
	if (0 <= sampler && sampler < away.core.display3D.Context3D.MAX_SAMPLERS) {
...
away.core.display3D.Context3D.prototype.setVertexBufferAt = function(index, buffer, bufferOffset, format) {
...
away.core.display3D.Context3D.prototype.setGLSLVertexBufferAt = function(locationName, buffer, bufferOffset, format) {
	bufferOffset = bufferOffset || 0;
	format = format || null;
	...
	var numBytes = 4;
	switch (format) {
		case away.core.display3D.Context3DVertexBufferFormat.BYTES_4:
			dimension = 4;
			break;
		case away.core.display3D.Context3DVertexBufferFormat.FLOAT_1:
			dimension = 1;
			break;
		case away.core.display3D.Context3DVertexBufferFormat.FLOAT_2:
			dimension = 2;
			break;
		case away.core.display3D.Context3DVertexBufferFormat.FLOAT_3:
			dimension = 3;
			break;
		case away.core.display3D.Context3DVertexBufferFormat.FLOAT_4:
			dimension = 4;
			break;
		default:
...
away.core.display3D.Context3D.prototype.updateBlendStatus = function() {
...
away.core.display3D.Context3D.className = "away.core.display3D.Context3D";

away.core.display3D.Context3D.getRuntimeDependencies = function(t) {
	var p;
	p = [];
	...
	p.push('away.core.display3D.CubeTexture');
	p.push('away.core.display3D.VertexBuffer3D');
	p.push('away.core.display3D.Context3DTriangleFace');
	p.push('away.core.display3D.SamplerState');
	...
	p.push('away.core.display3D.IndexBuffer3D');
	p.push('away.core.geom.Rectangle');
	p.push('away.core.display3D.Context3DCompareMode');
	return p;
};

away.core.display3D.Context3D.getStaticDependencies = function(t) {
	var p;
	return [];
};

away.core.display3D.Context3D.injectionPoints = function(t) {
	var p;
	switch (t) {
		case 0:
			p = [];
